<script lang="ts">
	...
	/**
	 * The customer's support form. Deliberately the contractor page's twin — same
	 * structure, same controls, same words — so there is one support form in this
	 * product with two accents, not two forms that drifted apart. The only real
	 * differences: no issue number comes back (the tracker is ours, not theirs),
	 * and the whole thing is read-only under view-as.
	 */
	...
	let title = $state('');
	let detail = $state('');
	const canSend = $derived(title.trim() !== '' && detail.trim() !== '' && !data.viewing);
...
<div class="wrap">
	...
	{#if !data.configured}
		<div class="notice">
			...
			<p>Please contact your contractor directly in the meantime.</p>
		</div>
	{:else if sent}
...
<style>
	/* Structurally the contractor support page. The only substitution is the
	   accent: `--accent` (the portal's teal) wherever that page uses --yellow. */
	.wrap {
		max-width: 640px;
		margin: 0 auto;
		padding: 0 0 1.5rem;
	...
	.field textarea {
		...
		border-radius: 10px;
		/* 16px+ so focusing a field never zooms the page on iOS. */
		font-size: 1rem;
		font-weight: 500;
		text-transform: none;
