...
<div style="max-width: 760px; margin: 0 auto; padding: 1rem; display: grid; gap: 1rem;">
	...
	{#if !data.active}
		<section
			style="border: 1px solid #d0d7de; border-radius: 16px; padding: 1.25rem; text-align: center; color: #57606a;"
		>
			<p style="margin: 0;">
				You don’t have an active order yet. When your contractor starts one, it will show up here.
			</p>
		</section>
	{:else}
		{@const active = data.active}
		<section
			style="border: 1px solid #d0d7de; border-radius: 16px; padding: 1rem; display: grid; gap: 0.5rem;"
		>
		...
		>
			...
			<div style="display: grid; gap: 0.65rem;">
				{#each active.timeline as entry (entry.id)}
					<div
						style="padding: 0.75rem; border-radius: 12px; background: #f6f8fa; border: 1px solid #d0d7de; display: grid; gap: 0.2rem;"
					>
						<strong>{entry.title}</strong>
						{#if entry.detail}<div style="font-size: 0.9rem; color: #57606a;">
								{entry.detail}
							</div>{/if}
		...
		>
			...
			<div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
				{#each Object.entries(requestLabels) as [value, label] (value)}
					<button
						type="button"
						onclick={() => (requestType = value as typeof requestType)}
						style="padding: 0.5rem 0.75rem; border-radius: 999px; border: 1px solid #d0d7de; background: {requestType ===
						value
							? '#0969da'
							: '#f6f8fa'}; color: {requestType === value ? '#fff' : 'inherit'};">{label}</button
					>
				{/each}
			</div>
			<form method="POST" action="?/request" use:enhance style="display: grid; gap: 0.5rem;">
				...
				<textarea
					name="detail"
					rows="3"
					placeholder={requestLabels[requestType]}
					required
					style="padding: 0.6rem; border-radius: 8px; border: 1px solid #d0d7de; resize: vertical;"
				></textarea>
				<button
					type="submit"
