...
/**
 * Account, and the support form that now lives on one of its tabs.
 *
 * Support was its own route in the portal's nav, the way it was on the
 * contractor side before it folded into Settings. Same reasoning applies here:
 * it is not somewhere a customer works, and in a portal whose whole nav is
 * "your projects" it was the one entry that had nothing to do with the job. It
 * is a tab on Account now, and `/customer/support` redirects onto it.
 *
 * The action is the customer half of the same mechanism the contractor's uses —
 * it files a GitHub issue — but tagged `from:customer` so the two audiences can
 * be told apart in triage. A customer reporting "the timeline is blank" is a
...
export const load: PageServerLoad = ({ locals }) => {
	...
	return {
		...
		// `viewAs` is not repeated here — the portal layout already loads it, and
