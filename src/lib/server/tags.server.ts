...
import { db } from './db';
import { order, subcontractor } from './db/schema';
 ...
 * from the tags actually in use across their orders and subcontractors — the same
 * rule the Guide follows (see
 ...
 *
 * Archived subcontractors and deleted orders still contribute. Their tags are part
 * of the contractor's vocabulary, and dropping a tag out of the suggestions just
 * because the last record using it was archived would be surprising.
 *
 * Customers carry no tags: a customer is identified by who they are, not by
 * labels, so the field was removed from that surface entirely.
 */
