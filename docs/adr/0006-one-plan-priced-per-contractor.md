...
correlates with value is _how many contractors are working_, not how many rows they hold.
...
- No code should branch on "which plan" — only on _is this Subscription live_. A `plan` column that
...
- Multi-seat companies (one bill, several Contractor logins) are deliberately deferred. Because pricing is
  ...
  flow — no repricing and no migration. Note that a seat invite would be the _third_ parallel invite
