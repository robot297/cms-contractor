# order-workspace Specification

## Purpose

Give contractors a dedicated workspace for their orders, separate from the dashboard. An order is described by a project name and type, is linked to a customer from the contractor's directory, and carries contractor-controlled follow-up scheduling plus internal notes. Follow-up is something the contractor sets explicitly — the system does not infer "needs attention" from an order's lifecycle state.
