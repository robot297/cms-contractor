# subcontractor-directory Specification

## Purpose

Let a contractor keep a roster of the subcontractors they work with. Subcontractor records are contractor-scoped and carry an administered profile (trade, contact, company, license, insurance, notes) plus an access Tier — `trusted` or `guest` — that governs what the subcontractor can see and do on every order they are assigned to. Contractors invite subcontractors by magic link; acceptance binds a user to the subcontractor record by token, subject to a one-role-per-user rule.
