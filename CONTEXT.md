...
**Directory**:
A Contractor's private, searchable list of everyone they know — the people they work FOR and the people they work WITH, in one place at `/contractor/people`. Its ONLY source is records the Contractor created. It was three lists (Customers, Crew, Subcontractors), which meant deciding what somebody was to you before you could look them up. "Select existing" means reusing one of your own repeat Customers on a new Order — never discovering another Contractor's Customers or self-registered accounts.

**Person** / **Role**:
One human in the Directory, and what they are to this Contractor. A Person is not a table: it is a read model that matches a Contractor's **Customer**, **Worker** and **Subcontractor** records **by email**, so the same Dave can be the Customer whose deck you rebuilt, the hand you call on Tuesdays, and the firm you engage — one row, three Roles, three records. The records stay separate because they carry different capability (an **Order** points at a Customer, an **Assignment** at a Worker or Subcontractor, and only a Subcontractor holds a **Tier**, insurance or a portal login); what is unified is identity, not storage. Somebody with no email is their own Person, because there is nothing to match on and matching by name would merge two different Daves. Adding a Role creates that record from what is already on file; removing one archives it and leaves the others alone. Where two records disagree about a shared field, the most recently updated one wins. See `mergeDirectory` in `src/lib/people.ts`, where the rule lives and is tested.
_Avoid_: Contact (ambiguous with the verb), party, entity; and _Person_ as a database noun — there is no person table.
...
**Payment**:
Money a **Customer** has handed a **Contractor** against one **Order**, recorded after the fact — a deposit, an interim payment, or the final one. Carries an amount, which of those three it was, how it was taken, and the date the money changed hands (which is the Contractor's to state, and is not the date it was typed in). The app **records** a Payment and never processes one: there is no card entry and no payment rail, on either surface. See [ADR-0010](docs/adr/0010-record-final-payment-never-process-it.md) and [ADR-0011](docs/adr/0011-payments-are-rows-and-the-balance-is-derived.md).
_Avoid_: Transaction, charge, receipt (a receipt is what the **Invoice** is called once it is settled), collecting/taking payment (the app does neither).

**Invoice**:
What an **Order** costs, what has been paid against it, and what is left — computed, never stored. The total lives on the Order and is agreed when the quote is; the **Payments** are rows; the balance is the subtraction. One calculation feeds all three places it appears — the Contractor's workspace, the Customer's **Portal**, and the emailed copy — so they cannot disagree. Once the balance reaches zero the same document calls itself a **receipt**.
_Avoid_: Bill, statement, balance-as-a-stored-field (it is derived, and storing it is the bug this arrangement exists to prevent).
...
Where the contractor app puts its links on a phone or tablet: _top_ (behind the bar's menu button, the original) or _bottom_ (a fixed tab bar at the foot of the screen, the shape the customer Portal has always had). A Contractor's own preference, stored on their settings row and applied by the contractor layout. It changes nothing above the desktop breakpoint, where the bar's rail navigates either way, and it changes no link, route or permission — the same five destinations either way.
_Avoid_: Layout, theme (both are other preferences), mobile mode.
...
- An **Order** holds many **Payments**, and a **Payment** belongs to exactly one Order. The Order carries the total; the Payments carry what came in; the balance is never stored.
- A **Payment** is only ever recorded by the **Contractor** — they are the one who saw the money arrive. A **Customer** sees their own Payments and balance on their **Portal**, and can record nothing.
- An **Order State** and its **Invoice** are independent. _Deposit Pending_ says what the Contractor is waiting for; the Invoice says what is actually owed, and neither is derived from the other.
...
- "Payments": resolved, in two steps, and the wording here was wrong after each of them. The **Order** lifecycle has always included states named _Deposit Pending_ and _Final Payment Pending_, and this entry used to say the product "only ever tracks [money] as a status — it never handles it", with the only real money being a Contractor's **Subscription**. [ADR-0010](docs/adr/0010-record-final-payment-never-process-it.md) made that too strong by adding a close-out record, and [ADR-0011](docs/adr/0011-payments-are-rows-and-the-balance-is-derived.md) makes it plainly wrong: an Order now carries a total, a list of **Payments** and a derived **Invoice**, shown to the Customer as well as the Contractor. The distinction that survives — and the one that matters — is **record versus process**: the app writes down money that changed hands somewhere else, and moves none of it. There is still no control in either surface that could be mistaken for a payment rail, which is the same reason the portal's **Make a payment** quick action was removed.
